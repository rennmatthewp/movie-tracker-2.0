...
describe('userReducer', () => {
  ...
  it('should return an updated favorites array on ADD_FAVORITE', () => {
    const initialState = { name: 'Gandalf', password: 'pointyhat', email: 'whitewizard@gmail.com', favorites: [] };
    const mockAction = {
      type: 'ADD_FAVORITE',
      film: {
  ...
  it('should return a favorites array without the film being removed on REMOVE_FAVORITE', () => {
    ...
    const expected = { name: 'Gandalf', password: 'pointyhat', email: 'whitewizard@gmail.com', favorites: [] }
    expect(userReducer(initialState, mockAction)).toEqual(expected);
  })
});
